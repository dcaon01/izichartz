...
export async function createProject(prevState, formData) {
    ...
    const size = formData.get("project-size");
    console.log("DIMENSIONE: " + size);
    const { width, height } = JSON.parse(size);
    console.log("DIMENSIONI: " + height + " " + width);
    ...
    const newProject = {
        status: "saved",
        selected: [],
        workpane: { width: width, height: height },
        zoom: 50,
        elements: [],
        errors: []
    }
    ...
    try {
        ...
        await client.query(insertProjectQuery, [id, name, value.email, module, newProject, date, date, "/previews/empty.png"]);
