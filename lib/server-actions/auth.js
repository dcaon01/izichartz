...
export async function userRegister(prevState, formData) {
    ...
    try {
        ...
        while (present) { // Controlliamo che lo slug non sia già presente.
            ...
            const encryptSlug = encrypt(slug);
            result = await client.query(verifySlugQuery, [encryptSlug]);
            if (result.rows.length === 0) {
                present = false;
                insertActCodValues.push(encryptSlug);
            }
        }
...
export async function userAccountActivation(prevState, formData) {
    ...
    const activateQuery = 'UPDATE "USERS" SET "active"=true WHERE email=$1';
    let result = null;
    ...
    try {
        await client.connect();
        const encryptSlug = encrypt(slug);
        result = await client.query(verifyQuery, [encryptSlug]);
        console.log(result.row[0].expires.getTime()); // debug
        // Verifichiamo che il code sia uguale (ovviamente per ora non lo hashamo ma dovremmo).
        if (!(code === result.rows[0].code)) {
            return messages.push("Code is not correct");
        }
        // Verifichiamo che il codice non sia scaduto.
        if (!(result.row[0].expires.getTime() < Date.now())) {
            return messages.push("Code is expired, please resend it.");
        }
        await client.query("BEGIN");
        await client.query(activateQuery, [user]);
        // Generare il session id, memorizzarlo nel db e inviarlo all'utente.
        await client.query("COMMIT");
    } catch (error) {
        console.log(error);
        messages.push("Something went wrong. Please, try again later.");
...
/**
 * validateSessionId
 * Funzione per la validazione del sessionId dell'utente.
 * Capire come criptare il sessionId magari con chiave pubblica e privata per una maggiore sicurezza.
 */
 ...
 */
function generateSessionId(lenght) {
