 ...
 * - Controllare se viene controllato che i codici siano univoci
 */
...
import cryptoJS from "crypto-js"; // Probabilmente è da eliminare perché non serve ad un cazzo
import { revalidatePath } from "next/cache";
import clientOpt from "@/lib/utility/pgClientOptions";
//import { Resend } from "resend";

// Encryption
const c_key = process.env.C_KEY; // Probabilmente inutile
...
export async function userRegister(prevState, formData) {
    ...
    try {
        ...
        const date = new Date(Date.now() + 43200000); // 12 ore
...
export async function userLogin(prevState, formData) {
    ...
    const client = new Client(clientOpt);
    let messages = [];
    const user = formData.get("login-user");
    const password = formData.get("login-password");
    // Verifichiamo che ci sia un utente già registrato(che deve essere attivo) nel database
    const findUserQuery = 'SELECT * FROM "USERS" WHERE "email"=$1 OR "username"=$2';
    const newSessionQuery = 'UPDATE "SESSIONS" SET "sessionId"=$1, "data"=$2, "expires"=$3 WHERE "user"=$4';
    const newActivation = 'UPDATE "VERIF_CODES" SET "code"=$1, "slug"=$2, "expires"=$3 WHERE "user"=$4';
    let email = "";
    let username = "";
    let sessId = "";
    let hashedSessId = "";
    ...
    try {
        await client.connect();
        result = await client.query(findUserQuery, [user, user]);
        if (result.rows.length === 0) {
            messages.push("No user found. Please, if you haven't done it yet, register.");
            return { messages };
        }
        // Confrontare la password
        const match = await bcrypt.compare(password, result.rows[0].psswrd);
        if (!match) {
            messages.push("The password is not correct.");
            return { messages };
        }
        // Attivare una nuova sessione nel database
        // Bisogna verificare che l'utente sia stato attivato
        email = result.rows[0].email;
        sessId = generateSessionId();
        hashedSessId = await hash(sessId);
        await client.query("BEGIN");
        if (result.rows[0].active === true) {
            // Differenziamo i due casi semplicemente perché le due sessioni
            // possono durare tempi diversi
            date = new Date(Date.now() + 43200000);
            data = {
                type: "session"
            };
            username = result.rows[0].username;
            redirection = `/workspace/${username}`;
        } else {
            date = new Date(Date.now() + 3600000);
            data = {
                type: "enroll"
            };
            // Creiamo il codice di attivazione (sarà da inviare per mail)
            const slug = generateRandomSlug(128);
            const code = generateRandomCode(27);
            await client.query(newActivation, [code, slug, date, email]);
            redirection = `/authentication/activation/${slug}`;
        }
        await client.query(newSessionQuery, [hashedSessId, data, date, email]);
        await client.query('COMMIT');
        client.end();
        cookies().set('sid', JSON.stringify(
            {
...
/**
 * userLogout
 * Server Action che permette di andare a sloggare l'utente.
 * @param prevState stato precedente, passato dall'hook useFormState.
 * @param formData dati passati dal submit del form linkato a questa server action.
 */
export async function userLogout() {
    const client = new Client(clientOpt);
    // Controllare se ha senso verificare che il client abbia inviato un cookie, 
    // anche se non credo
    const expireSessionQuery = 'UPDATE "SESSIONS" SET "expires"=$1 WHERE "user"=$2';
    const sid = cookies().get('sid');
    try {
        await client.connect();
        await client.query('BEGIN');
        const date = new Date(Date.now());
        await client.query(expireSessionQuery, [date, sid.value.email]);
        await client.query('COMMIT');
        client.end();
        cookies().delete('sid');
    } catch(error) {
