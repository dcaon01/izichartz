...
export async function userRegister(prevState, formData) {
    ...
    const insertQuery = 'INSERT INTO "USERS"("email", "username", "pswrd") VALUES($1, $2, $3)';
    const verifyUsernameQuery = 'SELECT "username" FROM "USERS" WHERE "username"=$1';
    const verifyEmailQuery = 'SELECT "email" FROM "USERS" WHERE "email"=$1';
    let result = null;
    ...
    /* Conversione per essere inseriti all'interno del DB */
    const userNameArray = Array.from(username);
    const emailArray = Array.from(email);
    const insertValues = [emailArray, userNameArray, Array.from(password)];
    ...
    try {
        ...
        result = await client.query(verifyUsernameQuery, [userNameArray]);
        console.log(result);
        if (result.rows.length > 0) {
            messages.push("Username already exists.");
        }
        result = await client.query(verifyEmailQuery, [emailArray]);
        if (result.rows.length > 0) {
